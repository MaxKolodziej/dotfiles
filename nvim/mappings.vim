...
command! Mappings E ~/dotfiles/nvim/mappings.vim
command! Keys E ~/dotfiles/nvim/mappings.vim
command! Settings E ~/dotfiles/nvim/settings.vim
command! LuaConf E ~/dotfiles/nvim/lua_config.lua
command! Manager E ~/dotfiles/nvim/plug.vim
...
function! ChangeClassToPath(string)
  return substitute(substitute(a:string, '_', '', 'g'), '::', '/', 'g')
endfunction

function! GetNamespaceUnderCursor()
  let l:word = expand('<cWORD>')
  let l:ns = matchstr(l:word, '\v[A-Za-z0-9_]+(::[A-Za-z0-9_]+)+')

  if l:ns !=# ''
    return ChangeClassToPath(l:ns)
  endif

  return ChangeClassToPath(expand('<cword>'))
endfunction

nmap fg :Telescope buffers<CR>
nmap ff :Telescope find_files<CR>
nmap fn :execute 'Telescope find_files default_text=' . GetNamespaceUnderCursor()<cr>
" original version that only expands to single word, not whole namespace
